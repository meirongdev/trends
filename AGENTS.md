...
Dependencies are vendored in the local module cache. **Always build and test offline** — the sandbox usually has no network:
...
| `internal/api` | Read-only REST API (`/healthz`, `/api/v1/...`) — being built in M1b |
...
- **Done (merged to `main`):** M0 (data foundation), M1a (scoring & rankings).
- **In progress:** M1b — read REST API (M1b-1 trending/languages/health; M1b-2 repo detail + search).
- **Next:** M2 (Next.js SSR frontend / SEO), M3 (badges, developer rankings, live mentions).
